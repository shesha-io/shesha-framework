...
namespace Shesha.DynamicEntities
{
    ...
    public class EntityConfigsBootstrapper : IBootstrapper, ITransientDependency
    {
        ...
        public async Task ProcessAsync()
        {
            ...
            var filteredAssemblies = assemblies.Where(x => !_startupSession.AssemblyStaysUnchanged(x)).ToList();
            Logger.Warn($"{all - filteredAssemblies.Count()} assemblies skipped as unchanged");
            ...
            using (var unitOfWork = _unitOfWorkManager.Begin())
            {
                using (_unitOfWorkManager.Current.DisableFilter(AbpDataFilters.SoftDelete))
                {
                    ...
                    var forceUpdate = _dbAllConfigs.Where(x => !x.IsDeleted && x.Source == MetadataSourceType.ApplicationCode).All(x => x.InheritedFrom == null)
        ...
        private async Task ProcessAsync(List<Type> entityTypes)
        {
            ...
            Logger.Info($"Found {entityTypes.Count()} entity types");
            ...
            Logger.Info($"Found {_dbAllConfigs.Count()} entity in the DB");
            ...
            Logger.Info(toUpdate.Any() ? $"Found {toUpdate.Count()} entities to update" : $"Existing entity configs are up to date");
            ...
            Logger.Info(toAdd.Any() ? $"Found {toAdd.Count()} entities to add" : $"New entities not found");
            ...
            foreach (var config in sortedToAdd)
            {
                ...
                ec.InheritedFrom = inheritedFrom
                ...
                // ToDo: AS - remove after implementing surface/expose feature
                ec.VersionNo = 1;
            ...
            foreach (var config in inherited)
            {
                ...
                else
                {
                    if (prop.DataType != property.DataType)
                    {
                        ...
                        throw new Exception($"Inheritance error from {property.EntityConfig.FullClassName} {property.Name} ({property.DataType}): {config.FullClassName} has property ({prop.DataType})");
                    }
                }
        ...
        )
        {
            try
            {
                ...
                foreach (var cp in codeProperties)
                {
                    ...
                    {
                        await UpdatePropertiesAsync(entityType, entityConfig, cp.Properties, dbProperties, dbp);
                    }
        ...
        private async Task UpdateItemsTypeAsync(EntityProperty dbp, PropertyMetadataDto cp)
        {
            ...
            {
                var itemsTypeProp = dbp.ItemsType ?? new EntityProperty() { EntityConfig = dbp.EntityConfig };
