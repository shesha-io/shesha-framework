...
alter table frwk.entity_configs add full_class_name citext GENERATED ALWAYS AS (coalesce(namespace, '') || '.' || coalesce(class_name, '')) stored
");
...
update 
	ep 
set 
	ep.entity_type = ec.class_name, 
	ep.entity_module = m.name
from 
...
SET 
    entity_type = ec.class_name,
    entity_module = m.name
FROM frwk.configuration_items ci,
     frwk.entity_configs ec,
     frwk.configuration_items cec,
     frwk.modules m
WHERE 
    ci.id = frwk.entity_properties.entity_config_id
    AND ((COALESCE(ec.namespace, '') || '.' || COALESCE(ec.class_name, '')) = frwk.entity_properties.entity_type
         OR ec.type_short_alias = frwk.entity_properties.entity_type)
    AND cec.id = ec.id
    AND m.id = cec.module_id
    AND frwk.entity_properties.entity_type IS NOT NULL
    AND frwk.entity_properties.entity_type != ''
    AND (frwk.entity_properties.data_type IN ('entity', 'file')
         OR frwk.entity_properties.data_format IN ('entity', 'file', 'interface'));
");
