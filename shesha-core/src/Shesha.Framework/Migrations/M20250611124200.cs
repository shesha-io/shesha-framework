...
{
    [Migration(20250611124200)]
    public class M20250611124200 : OneWayMigration
    {
        public override void Up()
        {
            ...
            if (!Schema.Table("Frwk_EntityProperties").Column("ListConfiguration").Exists())
                Alter.Table("Frwk_EntityProperties").AddColumn("ListConfiguration").AsStringMax().Nullable();

            if (!Schema.Table("Frwk_EntityProperties").Column("Formatting").Exists())
                Alter.Table("Frwk_EntityProperties").AddColumn("Formatting").AsStringMax().Nullable();
