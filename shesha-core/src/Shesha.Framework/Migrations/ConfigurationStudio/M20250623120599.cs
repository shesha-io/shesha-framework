...
namespace Shesha.Migrations.ConfigurationStudio
{
    ...
    public class M20250623120599 : OneWayMigration
    {
        ...
        {
            Execute.Sql("drop view vw_Frwk_PermissionedObjectsFull");

            Execute.Sql(@"CREATE OR ALTER VIEW frwk.vw_permissioned_objects_full
AS
with not_deleted as (
	select 
		id
...
from 
	...
	left join frwk.modules m on m.id = ad.module_id");
			...
			Execute.Sql(@"drop view vw_Core_ReferenceListItemValues");
			Execute.Sql(@"create view frwk.vw_reference_list_item_values as
select
    m.name as module,
    ci.name,
    item.item,
