...
{
    [Migration(20250623120999)]
    public class M20250623120999 : OneWayMigration
    {
        public override void Up()
        {
            Alter.Table("configuration_items").InSchema("frwk")
                   .AddColumn("exposed_from_id").AsGuid().Nullable().ForeignKey("fk_configuration_items_exposed_from_id", "frwk", "configuration_items", "id").Indexed();

            Execute.Sql(@"create or alter view frwk.vw_configuration_items_inheritance as
with cte as (
...
from 
	...
	inner join frwk.modules m on m.id = cte.module_id
	inner join frwk.modules om on om.id = cte.override_module_id");
        }
    }
}
