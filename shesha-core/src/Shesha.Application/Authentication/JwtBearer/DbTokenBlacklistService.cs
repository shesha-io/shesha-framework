...
namespace Shesha.Authentication.JwtBearer
{
    ...
    public class DbTokenBlacklistService : ITokenBlacklistService, ITransientDependency
    {
        ...
        public async Task<bool> TryBlacklistTokenAsync(string tokenId, DateTime? expirationDate)
        {
            // First check cache - if already blacklisted, return false immediately
            var cacheItem = await _cacheHolder.Cache.TryGetValueAsync(tokenId);
            if (cacheItem.HasValue && cacheItem.Value)
                return false;
            ...
            var rowsAffected = await BlacklistTokenInDbAsync(tokenId, expirationDate);
        ...
        private async Task<int> BlacklistTokenInDbAsync(string tokenId, DateTime? expirationDate)
        {
            using (var session = _sessionFactory.OpenSession())
            {
