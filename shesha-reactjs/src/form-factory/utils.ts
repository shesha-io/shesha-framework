...
type props = AllComponentProps | `${AllComponentProps}.${string}`;
...
export const getMetadataPropertyByProperty = (propertyName: string | undefined, defaultPropertyName: string | undefined = undefined): string | undefined => {
  if (defaultPropertyName) return `contexts?.formContext?.propMetadata?.${defaultPropertyName}`;
  if (!propertyName) return undefined;
  const metaProp = metadataToPropertyMap.find((m) => m.prop === propertyName)?.metaProp;
  if (!metaProp) return undefined;
  if (typeof metaProp === 'object' && 'code' in metaProp) return metaProp.code.replace('@metadata@', 'contexts?.formContext?.propMetadata?');
  return `contexts?.formContext?.propMetadata?.${metaProp}`;
};
