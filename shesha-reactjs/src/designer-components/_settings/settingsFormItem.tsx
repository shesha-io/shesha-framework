...
const SettingsFormItem: FC<ISettingsFormItemProps> = (props) => {
  ...
  if (settingsPanel) {
    ...
    if (!Boolean(propertyFilter)) propertyFilter = settingsPanel.getPropertyFilter();
  }

  return !Boolean(propertyFilter) ||
    (typeof propertyFilter === 'function' && propertyFilter(props.name?.toString())) ? (
    <Form.Item {...props} />
  ) : null;
};
