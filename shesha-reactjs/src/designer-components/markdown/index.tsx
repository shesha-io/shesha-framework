...
import { getStyle, useSheshaApplication, ValidationErrors } from '@/index';
...
const MarkdownComponent: IToolboxComponent<IMarkdownProps> = {
  ...
  Factory: ({ model }) => {
    ...
    const { font, style, border, background, shadow } = model;
  ...
  migrator: (m) => m
   ...
   .add<IMarkdownProps>(2, (prev) => ({ ...migratePrevStyles(prev, defaultStyles()) }))
  .add<IMarkdownProps>(3, (prev) => {
