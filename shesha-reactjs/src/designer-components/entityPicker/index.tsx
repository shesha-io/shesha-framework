...
const EntityPickerComponent: IToolboxComponent<IEntityPickerComponentProps> = {
      ...
      (value: any, args: any) => {
        if (model.valueFormat === 'entityReference') {
          return !!value
            ? {
                id: value.id,
                _displayName: getValueByPropertyName(value, displayEntityKey) ?? value._displayName,
                _className: model.entityType,
              }
            : null;
        }
        if (model.valueFormat === 'custom') {
  ...
  migrator: (m) =>
    m
      ...
      .add<IEntityPickerComponentProps>(10, (prev) => ({
        ...prev,
        desktop: { ...defaultStyles(prev) },
        mobile: { ...defaultStyles(prev) },
        tablet: { ...defaultStyles(prev) },
      })),
  ...
  getFieldsToFetch: (propertyName, rawModel) => {
    if (rawModel.valueFormat === 'entityReference') {
      return [
        `${propertyName}.id`,
        rawModel.displayEntityKey ? `${propertyName}.${rawModel.displayEntityKey}` : `${propertyName}._displayName`,
  ...
  validateModel: (model, addModelError) => {
    if (!model.entityType) addModelError('entityType', 'Select `Entity Type` on the Properties panel');
  },
};
