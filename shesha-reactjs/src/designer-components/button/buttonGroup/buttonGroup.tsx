...
import { getActualModel, getStyle, IApplicationContext, useAvailableConstantsData } from '@/providers/form/utils';
...
const createMenuItem = (
  ...
  const childItems =
    props.childItems && props.childItems.length > 0
      ? props.childItems
          .map((x) => prepareItem(x, props.readOnly))
          .filter(getIsVisible)
          ?.map((props) => createMenuItem(props, getIsVisible, appContext, prepareItem, form))
      : null;

  return isDivider
    ? { type: 'divider' }
    : getButtonGroupMenuItem(renderButton(props, props?.id, appContext, form), props.id, props.readOnly, childItems);
};
...
const InlineItem: FC<InlineItemProps> = (props) => {
  ...
  if (isGroup(item)) {
    const menuItems = item?.childItems
      ?.map((x) => prepareItem(x, item.readOnly))
      .filter((item) => getIsVisible(item))
      .map((childItem) =>
        createMenuItem(
          { ...childItem, buttonType: childItem.buttonType ?? 'link' },
          getIsVisible,
          appContext,
          prepareItem,
          form
    ...
    return (
      <Dropdown key={uuid} menu={{ items: menuItems }} disabled={item.readOnly}>
        <Button
...
export const ButtonGroupInner: FC<IButtonGroupProps> = (props) => {
  ...
  const isGroupVisible = (group: IButtonGroup, itemVibilityFunc: ItemVisibilityFunc): boolean => {
    if (!isVisibleBase(group)) return false;
  ...
  const getIsVisible = (item: ButtonGroupItemProps) => {
    if (isDesignMode) return true; // show visibility indicator
  ...
  const prepareItem: PrepareItemFunc = (item, parentReadOnly) => {
    if (item?.editMode === undefined) item.editMode = 'inherited'; // prepare editMode property if not exist for updating inside getActualModel
...
export const ButtonGroup: FC<IButtonGroupProps> = (props) => {
  return <ButtonGroupInner {...props} items={props.items} />;
};
