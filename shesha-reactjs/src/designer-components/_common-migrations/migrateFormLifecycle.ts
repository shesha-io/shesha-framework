...
const getAfterDataLoad = (onDataLoaded: string, initialValues?: IKeyValue[]): string => {
    ...
    const initialObjString = JSON.stringify(initialData, null, 4)
      ...
      .replaceAll("\"'", "'").replaceAll("'\"", "'")
      .replace("\"__shaFormData\": 0", "...Object.fromEntries(Object.entries(form.data || {}).filter(([_, v]) => v !== undefined && v !== ''))"); // add loaded form data, filtering empty strings and undefined
