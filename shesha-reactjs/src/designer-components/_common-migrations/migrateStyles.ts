...
import { IInputStyles, IStyleType } from "@/interfaces";

type ExtendedType = IStyleType & {
    block?: boolean;
...
export const initializeStyles = (prev, defaults?, screen?: 'desktop' | 'tablet' | 'mobile'): IStyleType => {
...
export const migratePrevStyles = <T extends ExtendedType>(prev: T, defaults?: IStyleType) => {
    ...
    const result: T = {
        ...prev,
        desktop: { ...initializeStyles(prev, defaults, 'desktop') },
        tablet: { ...initializeStyles(prev, defaults, 'tablet') },
        mobile: { ...initializeStyles(prev, defaults, 'mobile') },
    };
