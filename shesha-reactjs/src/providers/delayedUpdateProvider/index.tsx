...
import { DelayedUpdateProviderActionsContext, DelayedUpdateProviderStateContext, DELAYED_UPDATE_PROVIDER_CONTEXT_INITIAL_STATE, IDelayedUpdateStateContext } from './context';
...
const DelayedUpdateProvider: FC<PropsWithChildren<IDelayedUpdateProps>> = ({ children }) => {
    ...
    const addItem = (groupName: string, id: any, data?: any) => {
        ...
        if (Boolean(group)) {
            ...
            if (Boolean(item)) {
                setState({groups: [...state.groups.map((gr) => {
                    return gr.name === groupName 
                        ? {name: groupName, items: [...(gr.items.map((it) => {
    ...
    return (
        <DelayedUpdateProviderStateContext.Provider value={state}>
            <DelayedUpdateProviderActionsContext.Provider value={{
                addItem,
                removeItem,
                getPayload
            }}>
