...
import { QUERY_BUILDER_CONTEXT_INITIAL_STATE, QueryBuilderActionsContext, QueryBuilderStateContext } from './contexts';
...
const QueryBuilderProvider: FC<PropsWithChildren<IQueryBuilderProviderProps>> = ({ children, fields, id }) => {
  ...
  const fetchFields = (fieldNames: string[]) => {
    if (!meta?.metadata?.properties) return;

    const containers: string[] = [null /*to ensure that root is loaded*/];
    fieldNames.forEach((f) => {
      const idx = f.lastIndexOf('.');
      const container = idx === -1 ? null : f.substring(0, idx);
      if (containers.indexOf(container) === -1) containers.push(container);
    });

    const promises = containers.map((prefix) =>
      getContainerMetadata({ metadata: meta.metadata, containerPath: prefix }).then((response) =>
        getPropertiesFromMeta(response, prefix)
      )
    );
      ...
      results
        ...
        .forEach((r) => {
          ...
          properties.forEach((prop) => {
            if (!state.fields.find((p) => p.propertyName === prop.propertyName)) missingProperties.push(prop);
          });
        });
  ...
  const fetchContainer = (containerPath: string): Promise<IModelMetadata> => {
    ...
    promise.then((response) => {
      ...
      const missingProperties = properties.filter(
        (prop) => !state.fields.find((p) => p.propertyName === prop.propertyName)
      );
      if (missingProperties.length > 0) {
        const newFields = [...state.fields, ...missingProperties];
...
export { QueryBuilderProvider, useMetadataFields, useQueryBuilder, useQueryBuilderActions, useQueryBuilderState };
