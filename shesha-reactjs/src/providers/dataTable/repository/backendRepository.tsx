...
const createRepository = (args: ICreateBackendRepositoryArgs): IBackendRepository => {
  ...
  const getPropertyNames = (columns: IConfigurableColumnsProps[]): string[] => {
    const result: string[] = [];
    columns.forEach((col) => {
      const dataCol = col.columnType === 'data' ? (col as IDataColumnsProps) : null;
      if (dataCol && dataCol.propertyName) result.push(dataCol.propertyName);
    });
    return result;
  };
  ...
  const prepareColumns = (configurableColumns: IConfigurableColumnsProps[]): Promise<DataTableColumnDto[]> => {
    ...
    if (dataProperties.length === 0) return Promise.resolve([]);

    // fetch columns config from server
    return metadataDispatcher
      .getPropertiesMetadata({ modelType: entityType, properties: dataProperties })
      .then((response) => {
  ...
  const performDelete = (_rowIndex: number, data: any, options: IDeleteOptions): Promise<any> => {
    const id = data['id'];
    if (!id) return Promise.reject('Failed to determine `Id` of the object');
    ...
    return endpointResolver.then((endpoint) => {
      ...
      const url = useQueryString ? `${endpoint.url}?${qs.stringify({ id })}` : endpoint.url;
      const data = useQueryString ? undefined : { id };

      return mutator
        .mutate({ ...endpoint, url }, data)
        .then((response) => {
          return response;
        })
        .catch((error) => {
  ...
  const exportToExcel = (payload: IGetListDataPayload): Promise<void> => {
    let excelColumns = payload.columns.map<IExcelColumn>((c) => ({ propertyName: c.propertyName, label: c.caption }));
...
export function withBackendRepository<WrappedProps>(
  WrappedComponent: ComponentType<WrappedProps & IHasRepository & IHasModelType>,
  args: IWithBackendRepositoryArgs
): FC<WrappedProps> {
  return (props) => {
