...
export const Dropdown: FC<IDropdownProps> = ({
  ...
  const defaultValue = Array.isArray(defaultVal)
    ? defaultVal
    : defaultVal
      ? evaluateString(defaultVal, { formData, formMode, globalState }) || undefined
      : undefined;

  const value =
    typeof val === 'string' ? ((evaluateString(val, { formData, formMode, globalState }) ?? undefined) as any) : val;
  ...
  const outcomeValueFunc: OutcomeValueFunc = useCallback(
    (value: ReferenceListItemDto, args?: any) => {
      if (valueFormat === 'listItem') {
        return !!value ? { item: value.item, itemValue: value.itemValue } : null;
      }
      if (valueFormat === 'custom') {
  ...
  const getLabeledValue = useCallback(
    (value: any, options: ISelectOption<any>[]) => {
  ...
  const getOptionFromFetchedItem = useCallback(
    (fetchedItem: ReferenceListItemDto, args?: any): ISelectOption<any> => {
      const label = !!labelCustomJs
        ? executeExpression<string>(labelCustomJs, { value: fetchedItem }, null, (e) => {
            console.error(e);
            return 'unknown';
          })
        : fetchedItem.item;
  ...
  const getSelectValue = () => {
    ...
    return options
      ?.filter(({ value: currentValue }) => selectedValues.indexOf(currentValue) > -1)
      ?.map((x) => x.label)
      ?.join(', ');
  };
