...
const modifyType = (types: CoreTypes, typeName: string, modifier: TypeModifier): void => {
  const type: Type | undefined = types[typeName];
  if (type) {
...
for (const typeName in typeModifiers) {
  if (typeModifiers.hasOwnProperty(typeName) && isDefined(typeModifiers[typeName])) {
    modifyType(types, typeName, typeModifiers[typeName]);
  }
};

// remove proximity search
if ("proximity" in standardOperators)
  // @ts-expect-error Ignoring TS2790
...
const customDatetimeWidget: DateTimeWidget<Config> = {
  ...
  jsonLogic: (val, _, wgtDef): string => {
    return moment(val as MomentInput, (wgtDef as DateTimeWidget<Config>).valueFormat).format();
  },
};
