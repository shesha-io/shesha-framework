...
export const Tabs: FC<Omit<IWizardComponentProps, 'size'>> = ({ form, ...model }) => {
  ...
  const { buttonsLayout = 'spaceBetween', direction, isDynamic, labelPlacement, wizardType = 'default' } = model;
  ...
  const steps = visibleSteps?.map<IStepProps>(({ id, title, subTitle, description, icon, customEnabled }, index) => {
    ...
    return {
      ...
      ...iconProps,
      content: <ComponentsContainer containerId={id} dynamicComponents={isDynamic ? components : []} />,
    };
  });
