  ...
  const childItems =
    props.childItems && props.childItems.length > 0
      ? props.childItems
          ...
          .filter(getIsVisible)
          ?.map((props) => createMenuItem(props, size, getIsVisible, appContext, prepareItem))
      : null;

  return isDivider
    ? { type: 'divider' }
    : getButtonGroupMenuItem(renderButton(props, props?.id, size, appContext), props.id, props.readOnly, childItems);
};
...
const InlineItem: FC<InlineItemProps> = (props) => {
  ...
  if (isGroup(item)) {
    const menuItems = item.childItems
      ...
      .map((childItem) =>
        createMenuItem({ ...childItem, buttonType: 'link' }, size, getIsVisible, appContext, prepareItem)
      );
    return (
...
export const ButtonGroupInner: FC<IButtonGroupProps> = ({
  items,
  size,
  spaceSize = 'middle',
  isInline,
