...
export const getData = (dataSource: string, entityType: string, longitude: string, latitude: string, evaluatedFilters: string, customUrl: string, ownerId: string, layertype: LayerTypeKeys, boundary: string) => {
    const useGetAll = dataSource === 'custom' ? useGet : useEntitiesGetAll;

    const getAllProps = dataSource === 'custom'
        ? { path: customUrl + `?id=${ownerId}` || '', lazy: true }
        : { lazy: true };
    ...
    const queryParams = useMemo(() => {
        ...
        result.properties = ['id', ...Array.from(new Set(layertype == 'points' ? [longitude, latitude] : [boundary] || []))].join(' ');
...
export const getMapPoints = (item: string, marker: ILayerMarker[], key: LayerTypeKeys) => {
    ...
    if (Array.isArray(checkedValue?.markers)) {
        if (checkedValue.layertype == 'polygon' && key === 'polygon') {
            ...
            return [
                ...checkedValue.markers,
            ];
        } else
            if (checkedValue.layertype == 'points' && key === 'points') {
                return [
