...
const DefaultDataDisplayCell = <D extends object = {}, V = number>(props: IDataCellProps<D, V>) => {
    ...
    switch (columnConfig.dataType) {
        case 'number': return (<NumberCell<D, V> {...props} />);
        case 'date': return (<DateCell<D, V> {...props} />);
        case 'date-time': return (<DateTimeCell<D, V> {...props} />);
        case 'time': return (<TimeCell<D, V> {...props} />);
        case 'reference-list-item': return (<ReferenceListCell<D, V> {...props} />);
        case 'boolean': return (<BooleanCell<D, V> {...props} />);
        case 'entity': return (<EntityCell<D, V> {...props} />);
        case 'array': {
            return columnConfig.dataFormat === 'reference-list-item'
                ? <MultivalueReferenceListCell<D, V> {...props} />
                : <StringCell<D, V> {...props} />;
        }
        case 'string': return <StringCell<D, V> {...props} />;
        default: return <StringCell<D, V> {...props} />;
    };
};
