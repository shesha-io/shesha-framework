...
function getItem({ label, key, icon, children, isParent, itemType, onClick, navigationType, url, tooltip }: IGetItemArgs): MenuItem {
  ...
  return {
    ...
    children,
    label: onClick
      ? (<Space direction='horizontal' size={'small'} title={tooltip && tooltip as string}>
          ...
          {tooltip && <QuestionCircleOutlined size={4} title={tooltip && tooltip as string} />}
        </Space>)
      : <Space direction='horizontal' size={'small'} title={tooltip && tooltip as string}>{<span className={className}>{label}</span>}{tooltip && <QuestionCircleOutlined size={4} title={tooltip && tooltip as string} />}</Space>,
